...
> What is the average number of discharges with a Subaracahnoid Haemorrhage diagnosis by age group in the East region of Scotland for all admissions in the finanical year 2019/20 and 2020/21?
...
## a more advanced and elegant solution is to using the across and where functions to coerce all characters to be factors since all of the variables we wanted to change were characters. Understanding data types is useful! 
stroke |> 
...
The output print out for `FinancialYear` and `HBName` are cut off as there are many factor levels (more than 6), so we will check these variables individually. Otherwise, `AdmissionType`, `AgeGroup`, and `Sex` all have aggregate response levels and `HBName` has `NA`s. 

```{r task-5-financialyear}
stroke |> 
  pull(FinancialYear) |> 
...
# We can confirm the number of missing values using Truthy and Falsey to our advantage 
## we can look for the total number of NA values using is.na and then sum 
...
Within `HBName` there are 14 health boards as expected but what could the `NA`s be? Well, we know from last week that `Hbr` in the stroke data set included all of Scotland with the country code S92000003. Because we still have the unedited `stroke_raw` data, we can check if these missing values match up to the aggregate country level. Having a raw data frame version is super useful! 
...
* `Diagnosis` only of Subaracahnoid Haemorrhage
...
stroke_q <- stroke |> 
  filter(AdmissionType == "All", 
         Diagnosis == "Subaracahnoid Haemorrhage",
         Sex == "All",
...
Another option is to combine the HBName factor levels. This requires us to first create a HBName column with collapsed factor levels. It is important to remember to then use this variable in later functions! 

```{r task-8-4}
stroke_q |>
  mutate(HBName_group = fct_collapse(HBName,
                                     EastRegion = c("NHS Lothian", "NHS Fife",
                                              "NHS Borders"))) |>
  group_by(HBName_group, AgeGroup, FinancialYear) |> 
  # notice the pipe from mean to round to then round the mean values, all within the summarise function 
  summarise(mean_discharge  = mean(NumberOfDischarges) |> round(3)) |>
  pivot_wider(values_from = mean_discharge,
              names_from = FinancialYear)
```
...
You have completed all of the tasks for the R notebook for this tutorial. If you have not done so yet, now move to the Python notebook.
