...
length(vec) # length meaning how many elements are in the vector 
...
💡 The hierarchy of data types in R are logical -> integer -> numeric -> character, where -> can be read as "are transformed into".

All logical values could be numeric or character, but not in the other direction - not all numeric or character values could be logical. Just as not all character values could be numeric, though all numeric values could be a character string. For this reason, when you include a character in a vector, the rest of that vector is parsed as a character string in order to keep a homogeneous data type in the data structure. 

Following this hierarchy, it is not surprising then that `vec` above is of type double because the single double value 34.5 coerced the whole vector to be of type double. 
...
`vector["name"]` is a way to call up a specific element of a named vector.
...
**♻️ Recycling** can be a very big problem within data analysis. Recycling is what happens when performing operations on vectors that do not have the same length. Both `vec` and `vec4` have a length of 5. Let's create a vector with length of 10 and consider what happens: 
...
We can create an array with the `array()` function. To specify multiple dimensions, we need to pass a vector to the `dim` argument.  
...
Notice that the print out is suddenly structured differently! This is because arrays, as mentioned above, can be multidimensional. The first and second number in the bracket specifies the amount of rows and columns. The last number in the bracket specifies how many dimensions we want. So taken all together, we get an array with 2 rows and 3 columns 4 times over. 
...
vec_array <- array(vec5, dim = c(3, 2, 5))
# creates a 3 x 2 x 5 array -- so 3 rows, 2 columns, and 5 in the 3rd dimension 
...
📝 🌍 If you look in your environment tab in RStudio and find `vec_array` listed, you will find the same preview as the output of `str(vec_array)`. The environment tab allows us to see all of the objects in our global working environment in this session of R and a wee preview of what those objects contain. As we go through this notebook, take a note of the environment as it will be reorganized when we move to lists and dataframes. 
...
The code on line 317 words as it accesses the 1st row and 1st column of the 2nd array. 


```{r array-replace}
# replace value 10 with 100 
## first look up index of 10 so [1, 2, 2]
vec_array[1, 2, 2] <- 100
...
In the second argument of `apply()` (`MARGIN`), in addition to specifying one dimension, you can also supply a vector of dimensions that the function will be applied upon. For example, the following code computes the sum along the first two dimensions
...
Returning to the print out of `list1`, the double and single square brackets are helpfully presented. Can you see how why `list1` has a length of 4? 
...
📌 🌍 Notice our global environment tab again. You will find both `slice` and `slice2` under "Data" but `index` under "Values". Look at the objects in your global environment to confirm that `index` returned a character string vector, while `slice` and `slice2` returned list objects. This is because when slicing a list, a list is returned but using the double square brackets we are returned with a vector! Can you notice this as well from the difference in how the objects in the `list-slice-index` code chunk are printed out?
...
list4 <- list(c("blue", "green", "yellow"), 
              list("lightblue", "lightgreen","lightyellow"))
...
To delete elements from a named list, we can use the `within()` function and `rm()` function, where rm means "remove".
...
R is unique in the sense that dataframes are a built-in data structure, but if we remember that the language was purpose built for data science and statistics as discussed in Week 1, it becomes less surprising.  
...
As mentioned in this week's content in topic 5, the `{tidyverse}` ecosystem uses `tibbles` in place of the base R `data.frame`. Tibbles have an enhanced print method, never coerce strings to factors, and provide stricter subsetting methods. Both data frames and tibbles are built on top of lists (which are built on top of vectors). 
...
# be sure to modify the code above to fit your own folder structure/file path to where you put the data file if you are not pulling notes from the course GitHub repo 

## notice that when we read in data with read_csv, we get a helpful message with some information about how the data was parsed (AKA read in). There are 3313 rows and 6 columns, 2 of which were parsed as a character data type and 4 as a double

# as I used the tidyverse read_csv function, our data is a tibble specifically 
is_tibble(gap_data)
```

As we learned last week, we can get some descriptions of our data frame with `str()` and `glimpse()`. We can also use `summary()`. The choice of which data frame describing function to use is up to you, and really depends on the type of information you are after. 

To view the data we can use `View()` or click on the data object in the global environment pane, which will open a spread-sheet like data viewer (notice the spreadsheet icon). ➡️ Try it now! 
...
When slicing data frames in R, the square bracket notation works as follows, where dimension 1 is the row and dimension 2 the column:
...
Some useful functions within a `summarise()` call include: 
...
`filter()` and `select()` allow you to subset your data frame by rows or columns specifically. 

* `filter()` keeps rows that match a condition
* `select()` keep or drop columns using their names or types 

Within `filter()` we can use the comparison and logical operators that we learned about in Week 2 & 3 Topic 2. Additionally, we can use the `%in%` operator to identify if a value or set of values appears within another set of values. The `%in%` operator in R is used to check if the values of the first argument are present in the second argument (typically a vector) and returns a logical vector indicating if there is a match or not for its left operand. 
...
💡 Remember, `mutate()` to modify the data, `summarise()` to  condenses the data into a minimal table that only includes the variables specifically asked for. 

📝 Note, you can overwrite existing columns with `mutate()`, but be careful when doing so.  
...
## click the arrow on the right of the tibble print out to see the added column 
```
...
We can also combine both of these steps into a single data flow with the pipe operator: 
...
To replace the `NA` values with the word "missing" in a character variable, we can use `replace_na()` within a `mutate()` function. To exclude rows with `NA`s, we can use the not operator `!` in front of the `is.na()` function within a `filter()` function. 
...
## OR to amend the existing column you can overwrite it - but again, be careful and intentional when doing this 
patients |>
