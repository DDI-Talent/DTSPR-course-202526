...
A right join is essentially the opposite of a left join, keeping all of the data from `appt_details` (the second argument, or data on the "right"). Thus, IDs `6` and `7` have `NA` for `age` as these IDs are not in the `patient_details` tibble.
...
A great question. In this case, we just need to specify using the `by = join_by()` parameter which column name in tibble1 is equal to in tibble2. For example, `by = join_by(a == b)` will match column a in tibble x to column b in tibble y. The name of the linkage key column in the first input data frame is what will be kept in the resulting data frame. If you want to keep both linkage key columns in the resulting data frame, you need to specify the argument `keep = TRUE`.
...
patient_details2 |> 
  full_join(appt_details, 
            by = join_by(patient == id), 
            keep = TRUE)
