...
Tasks 3, 4, and 5 are a bit more tricky. To really stretch yourself, consider using code to produce/print the answer to the above questions once you have a solution.
...
**Hint 4** How to use code to produce the answer? There are at least 2 ways we have touched on that you could do this with R. (1) using RMarkdown features or (2) using string interpolation which we learned about this week in the R string data types notebook. 
...
`Marital` is an unordered factor (nominal categorical data type) comprised of 6 levels. It makes sense to keep `marital` as a nominal categorical data type, though having "Never married" sorted as the first level then "Married" followed by "Separated", "Divorced", "Widowed", and "No answer" would make sense for some data presentations. 
...
Note this question is *not asking you to write code*, but rather work through the problem decomposition process and write down the logical steps.
...
Remember, in R, there are many ways to do the same thing. As long as your code does what you want, you can then work on understanding what your code is doing and why. Below is the solution I came up with:
...
This is a big one, so I have separated the task into different parts (i.e., done the problem decomposition step). By the end, you will have made a countdown clock to your birthday! (how cool!)

We will start by making a countdown clock until the annual Fringe Festival here in August. The festival starts on 7 August 2026 at 11:10. 
...
```{r fringedatatime}
Fringe_date <- dmy_hm("7 August 2026 11:10", tz = "Europe/London")
...
**Hint 3** Use one of the `{lubridate}` functions to get the date and time now, rather than hard coding it  
...
```{r countdown-output}
str_glue("Countdown to Fringe 2026: {countdown_day} days")
...
```{r BB-bday}
birthday <- mdy("July 4 2026", tz = "Europe/London")
...
I have named the fileswith 2 at the end so that should you run the code, you will see the new files saved in the data folder rather than overwriting the existing files. 

Notice also that in the file name in both cases, I have written the file path such that the CSVs are saved in our data folder! Rather than in the R folder. Remove the file path if you would like to experiment with this. 
...
write_csv(forcats::gss_cat |> select(-denom), file = "../data/gss_cat2.csv")
```
